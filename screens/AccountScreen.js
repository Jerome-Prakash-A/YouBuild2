...
export default function AccountScreen({navigation}) {
  ...
  const LogOutsetAsyncData = async() => {
    try{
        ...
        try{
          ...
          .then(value => {
            if(value!=null){
              console.log(value, 'Accountlogout');
            }else{
              console.log(value);
            }
          })
  ...
  return (
    <View style={styles.wholeScreen}>
      ...
      <ScrollView style={{ flex:1}} showVertivalScrollIndicator={false}>
          {/* My Profile */}
        <TouchableOpacity style={styles.MyProfile}>
          <View  style={styles.PhotoandDetails}>
            ...
            < Entypo name="chevron-thin-right" size={19} style={{paddingRight:8}} />
            
          </View>
          <View style={styles.progressContent}>
            <View style={{backgroundColor:'#EAE5E5', height:1, marginBottom:9}}></View>
            <Text style={{fontSize:11, color:'grey',marginBottom:3 }}>5 UBs left to complete this level!</Text>
            <Progress.Bar progress={0.5} borderColor={'white'} unfilledColor={'#ECE9E9'} color={'#ffdb00'} width={337} style={{ marginTop:3, marginBottom:9}} />
          </View>
